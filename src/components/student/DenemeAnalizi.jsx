/**
 * 📊 Deneme Analizi görünümü (Tasarım 2.0 — referans düzeni)
 *
 ...
 *
 * YERLEŞİM (referans): üstte ölçüm şeridi + KOMBO grafik (çubuk =
 * doğru/yanlış hacmi, çizgi = toplam net — "çok soruya dokunduğum
 * deneme iyi mi geçti?" sorusu tek grafikte), altında İKİ SÜTUN:
 * solda geçmiş ve seriler, sağda teşhis (D/Y/B, güçlü/zayıf, konu
 * hataları, nedenler, öncelikler).
 *
...
export default function DenemeAnalizi({ ogrenci, studentId, bakis = 'ogrenci', sinavTuru = 'all' }) {
    ...
    const sonDegisim = seri.length >= 2
        ...
        : null;
    const ortalamaNet = seri.length
        ? +(seri.reduce((a, d) => a + d.toplamNet, 0) / seri.length).toFixed(2)
        : null;
    const toplamHata = konu.konular.reduce((a, k) => a + k.sayi, 0);
    ...
    return (
        <div className="space-y-4">
            ...
            {/* ── ÖLÇÜM ŞERİDİ — referansın üst KPI satırı ─────────
                Renk yalnız ikonda ve anlam taşıyor; sayının şekli
            ...
            {seri.length > 0 && (
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
                    <OlcumKarti
                        ...
                        alt={seri[seri.length - 1].ad}
                        seri={seri.map((x) => x.toplamNet)}
                        seriTuru="dolgu"
            ...
            {/* ── KOMBO GRAFİK — referansın ana grafiği ────────────
                Çubuklar hacim (doğru/yanlış soru sayısı), çizgi sonuç
                (toplam net). İkisi aynı soru ölçeğinde olduğu için tek
                eksen yeter; ayrı eksen iki seriyi yapay biçimde
                birbirine yaklaştırırdı. */}
            {kombo.length >= 2 && (
                <div className="bg-surface border border-line rounded-2xl p-4 sm:p-5">
                    <h4 className="tip-h4 flex items-center gap-2">
                        <BarChart2 size={16} className="text-brand" /> Deneme Gelişimi
                    </h4>
                    <p className="tip-caption mt-0.5 mb-3">
                        Çubuklar doğru/yanlış sayısı, çizgi toplam net{sonDegisim !== null && (
                            <> · son denemede <strong style={{ color: sonDegisim >= 0 ? 'var(--ok)' : 'var(--danger)' }}>
                    ...
                    <div className="h-64">
                        <ResponsiveContainer width="100%" height="100%">
                            <ComposedChart data={kombo} margin={{ top: 4, right: 8, bottom: 0, left: -18 }}>
                                ...
                                <Tooltip contentStyle={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 12, fontSize: 12 }}
                                    labelFormatter={(deger, yuk) => yuk?.[0]?.payload?.ad || deger} />
                                <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 11 }} />
                                <Bar dataKey="dogru" name="Doğru" stackId="dyb" fill="var(--ok)" fillOpacity={0.85} radius={[0, 0, 0, 0]} maxBarSize={26} />
                                <Bar dataKey="yanlis" name="Yanlış" stackId="dyb" fill="var(--danger)" fillOpacity={0.75} radius={[4, 4, 0, 0]} maxBarSize={26} />
                                <Line type="monotone" dataKey="toplamNet" name="Toplam Net" stroke="var(--brand)" strokeWidth={3} dot={{ r: 4, fill: 'var(--brand)' }} activeDot={{ r: 6 }} />
                            </ComposedChart>
                        </ResponsiveContainer>
                    </div>
                </div>
            ...
            {/* ── İKİ SÜTUN — solda geçmiş ve seriler, sağda teşhis ──
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-4 items-start">
                <div className="xl:col-span-7 min-w-0 space-y-4">
                    {/* DENEME GEÇMİŞİ — referanstaki "Browse test results".
                        Satırdaki mini çubuk o denemenin en iyi denemeye
                        göre yerini gösterir. */}
                    {seri.length > 0 && (
                    ...
                    {/* DERSLERE GÖRE NET GELİŞİMİ — "matematiğim yükseliyor
                        mu?" sorusunun grafiği. Toplam netle AYNI grafikte
                        çizilmiyor: toplam 80-120 bandında, tek ders 5-25
                        bandındadır; aynı eksende ders çizgileri dibe yapışır.
                ...
                <div className="xl:col-span-5 min-w-0 space-y-4">
                    {/* DERSLERE GÖRE D / Y / B — çok segmentli çubuk.
                        Çubuklar aynı ölçekte olduğu için hem oran hem
                        hacim tek bakışta okunur. Renkler durum renkleri:
                        gösterilen şey ders kimliği değil, cevabın
                        doğru/yanlış/boş oluşudur. */}
                    ...
                    {/* Öğrencinin kendi deneme kayıtları — silme buradan */}
                    {manuelKayitlar.length > 0 && (
                        <Bolum baslik={`Deneme Kayıtlarım (${manuelKayitlar.length})`} ikon={ClipboardList}>
