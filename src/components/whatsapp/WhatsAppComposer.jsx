...
const WhatsAppComposer = ({
    ...
    /**
     * Veli portalı bağlantıları SUNUCUDAN hazırlanır.
     *
     * Şablonlardaki {veliLinki} eskiden koçun localStorage'ındaki ölü
     * belirteçten üretiliyordu; veliye gönderilen her bağlantı
     * "Öğrenci Bulunamadı" veriyordu. Artık seçili öğrenciler için gerçek
     * bağlantı önceden üretilir ve aynı anda velinin göreceği rapor özeti
     * de yayınlanır.
     */
    const [veliLinkleri, setVeliLinkleri] = useState({});
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            for (const s of selectedStudents) {
                ...
                const r = await veliBaglanti.baglantiAl(s, buildStudentReport);
                if (r.basarili) harita[String(s.id)] = r.adres;
            }
            if (!iptal) setVeliLinkleri(harita);
        })();
        return () => { iptal = true; };
    }, [selectedStudents]);
    ...
    const buildMessageFor = useCallback(
        ...
        [reports, coachName, extras, customBody, veliLinkleri]
    );
    ...
    return (
        ...
        >
            ...
            {/* ── Başlık ─────────────────────────────── */}
            <div className="shrink-0 flex items-center justify-between px-5 py-4 border-b border-line bg-gradient-to-r from-accent/20 to-transparent">
            ...
            {/* ── Adım göstergesi ────────────────────── */}
            <div className="shrink-0 flex items-center gap-1 px-5 py-3 border-b border-line">
                {STEPS.map((s, i) => (
